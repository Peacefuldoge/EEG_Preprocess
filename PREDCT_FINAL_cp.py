...
def _label_status_blocks_by_alpha(
    ...
    """
    Recover EO/EC labels for STATUS-only PRED+CT files.

    PRED+CT's explicit labelled recordings establish the canonical trigger map:
      1, 3, 5 -> Eyes Closed
      2, 4, 6 -> Eyes Open

    Trigger code is therefore the primary label source. Posterior alpha is used
    only as a physiological validation:
      - EC/EO alpha ratio >= ALPHA_INFERENCE_MIN_RATIO -> passed
      - neither direction differs enough -> inconclusive warning; keep triggers
      - EO alpha is clearly higher by the threshold -> error, because this
        contradicts the expected physiology and should be inspected manually

    This avoids dropping valid STATUS-only recordings merely because their
    alpha contrast is weak (for example ratio=1.073).
    """
    blocks = blocks.copy()
    ...
    codes = [
        int(x)
        for x in blocks["event_code"]
    ]

    unsupported = sorted(
        set(codes) - set(trigger_map)
    )

    if unsupported:
        raise ValueError(
            "STATUS blocks contain unsupported canonical trigger codes: "
            f"{unsupported}"
        )
    ...
    if (
        np.any(~np.isfinite(scores))
        or np.any(scores <= 0)
    ):
        raise ValueError(
            "Cannot validate STATUS EO/EC labels: invalid alpha scores."
        )

    blocks["posterior_relative_alpha"] = scores

    eo_mask = (
        blocks["event_label"].to_numpy()
        == "Eyes Open"
    )
    ec_mask = (
        blocks["event_label"].to_numpy()
    ...
    if (
        not np.any(eo_mask)
        or not np.any(ec_mask)
    ):
        raise ValueError(
            "STATUS trigger mapping did not produce both EO and EC blocks."
        )
    ...
    if ec_over_eo >= ALPHA_INFERENCE_MIN_RATIO:
        validation = "passed"
    elif eo_over_ec >= ALPHA_INFERENCE_MIN_RATIO:
        raise ValueError(
            "STATUS trigger labels contradict posterior-alpha validation: "
            f"EO alpha={eo_alpha:.6f}, EC alpha={ec_alpha:.6f}, "
            f"EO/EC={eo_over_ec:.3f} >= "
            f"{ALPHA_INFERENCE_MIN_RATIO:.3f}. "
            "Please inspect this recording manually."
        )
    else:
        validation = "inconclusive"
        warnings.warn(
            "STATUS trigger labels retained, but posterior-alpha validation "
            "is inconclusive: "
            f"EC/EO ratio={ec_over_eo:.3f} < "
            f"{ALPHA_INFERENCE_MIN_RATIO:.3f}. "
            "Trigger-code labels are used.",
            RuntimeWarning,
        )

    blocks["alpha_ec_median"] = ec_alpha
    blocks["alpha_eo_median"] = eo_alpha
    blocks["alpha_ec_over_eo_ratio"] = ec_over_eo
    blocks["alpha_validation"] = validation
    blocks["event_label_method"] = (
        "status_trigger_code+alpha_validation"
    )
    ...
    for label in blocks["event_label"]:
        ...
        counters[short] += 1
        ids.append(
            f"{short}_block_{counters[short]:02d}"
        )
...
def process_one_recording(
    ...
    # 11) Split EO/EC, reject severe 1-s artifacts within each state,
    #     then compute FC only from windows that pass QC.
    state_specs = [
    ...
    for state_short, state_label, state_output_path in state_specs:
        ...
        state_counts_raw[state_short] = raw_count

        if raw_count == 0:
            state_counts[state_short] = 0
            state_removed[state_short] = 0
            print(
                f"  [WARN] No complete {state_label} windows: "
                f"{set_path.name}"
            )
            continue
        ...
        keep_local, qc_info = (
            reject_artifact_windows(
                raw_state_data,
                starts=raw_state_starts,
            )
        )

        kept_indices = state_indices[
            keep_local
        ]

        state_counts[state_short] = int(
            len(kept_indices)
        )

        state_removed[state_short] = (
            raw_count
            - state_counts[state_short]
        )

        state_qc[state_short] = qc_info

        if state_counts[state_short] == 0:
            print(
                f"  [WARN] No {state_label} windows survived QC: "
                f"{set_path.name}"
            )
            continue

        state_blocks = event_blocks[
            event_blocks["event_label"]
            == state_label
        ].copy()

        # When resuming a partial batch, keep an existing state file unless
        # overwrite is enabled.
        if (
            state_output_path.exists()
            and not OVERWRITE
        ):
            state_counts[state_short] = (
                count_existing_npz_windows(
                    state_output_path
                )
            )
            continue
        ...
        (
            ...
            ends[state_indices],
            labels[state_indices],
            event_codes[state_indices],
            event_block_ids[state_indices],
            event_block_starts[state_indices],
            event_block_ends[state_indices],
        )

        state_fc = compute_pearson_fc(
            state_data
        )
        ...
        save_npz(
            ...
            ends=state_ends,
            labels=state_labels,
            event_codes=state_event_codes,
            event_block_ids=state_block_ids,
            event_block_starts=state_block_starts,
            event_block_ends=state_block_ends,
            subject_id=subject_id,
            run_id=run_id,
    ...
    result = {
        ...
        "n_windows_total_raw": int(data.shape[0]),
        "n_windows_total": int(
            state_counts.get("EO", 0) + state_counts.get("EC", 0)
        ),
        ...
        "line_freq": line_freq,
        "eyes_open_windows_raw": state_counts_raw.get("EO", 0),
        "eyes_open_windows": state_counts.get("EO", 0),
        "eyes_open_windows_removed": state_removed.get("EO", 0),
        "eyes_closed_windows_raw": state_counts_raw.get("EC", 0),
        "eyes_closed_windows": state_counts.get("EC", 0),
        "eyes_closed_windows_removed": state_removed.get("EC", 0),
        "window_qc_removed_total": (
            state_removed.get("EO", 0) + state_removed.get("EC", 0)
        ),
        ...
        try:
            ...
            if result.get("status") == "SKIPPED_EXISTS":
                ...
                tqdm.write(
                    ...
                    f"removed={result.get('window_qc_removed_total', 0)} | "
                    f"FC=64x64"
                )
            else:
                success_count += 1
